...
`http` is a thin, practical wrapper over the included Odin HTTP package.
...
- `http` - routers, handlers, responses, cookies, middleware, rate limits.
- `http/client` - client requests and responses.
- `http/session` - cookie session and CSRF planning.
- `http/sse` - server-sent events.
- `http/datastar` - Datastar patch events over SSE.
...
```clojure
(import session "deps/http/session")
...
`http/client` mirrors the explicit lifetime style:

```clojure
(import client "deps/http/client")
...
```clojure
(import dstar "deps/http/datastar")
...
- [`examples/http-server.kvist`](../examples/http-server.kvist) - small
  router and JSON response.
- [`examples/http-client.kvist`](../examples/http-client.kvist) - client
  GET and POST request setup.
- [`examples/http-session.kvist`](../examples/http-session.kvist) -
  session planning.
- [`examples/http-sse.kvist`](../examples/http-sse.kvist) - short SSE
  stream.
- [`examples/http-sse-live.kvist`](../examples/http-sse-live.kvist) -
  long-lived SSE stream.
- [`examples/http-datastar.kvist`](../examples/http-datastar.kvist) -
  Datastar events.
